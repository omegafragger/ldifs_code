...
def get_dataset(dataset_name, preprocess, location, image_text=False, batch_size=128, num_workers=16, val_fraction=0.1, max_val_samples=20000):
    if dataset_name.endswith('Val'):
        ...
        if dataset_name in registry:
            ...
            base_dataset = get_dataset(base_dataset_name, preprocess, location, image_text, batch_size, num_workers)
    ...
    dataset = dataset_class(
        preprocess, location=location, image_text=image_text, batch_size=batch_size, num_workers=num_workers
    )
    return dataset